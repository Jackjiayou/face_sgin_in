...
class WAS(wx.Frame):
    def __init__(self):
        wx.Frame.__init__(self,parent=None,title="人脸识别签到系统（韩思工作室）",size=(920,560))
    ...
    def initMenu(self):
        ...
        self.icon = wx.Icon(name="demo_icon.ico",  type=wx.BITMAP_TYPE_ICO)
        ...
        self.new_register.SetBitmap(wx.Bitmap("resources/new_register.png"))
        ...
        self.finish_register.SetBitmap(wx.Bitmap("resources/finish_register.png"))
        ...
        self.start_punchcard.SetBitmap(wx.Bitmap("resources/start_punchcard.png"))
        ...
        self.end_puncard.SetBitmap(wx.Bitmap("resources/end_puncard.png"))
        ...
        self.open_logcat.SetBitmap(wx.Bitmap("resources/open_logcat.png"))
        ...
        self.close_logcat.SetBitmap(wx.Bitmap("resources/close_logcat.png"))
        ...
        menuBar.Append(logcatMenu,"&签到日志")
    ...
    def OnFinishRegister(self):
        ...
        if self.pic_num>0:
            ...
            if len(feature_list) > 0:
                ...
                self.infoText.AppendText("工号:"+str(self.id)
                ...
                for i, knew_face_feature in enumerate(self.knew_face_feature):
                    ...
                    if compare == "same":  # 找到了相似脸
                        ...
                        if nowdt[nowdt.index(" ")+1:-1] <= self.puncard_time:
                            self.infoText.AppendText(self.knew_name[i] + " 成功签到,且未迟到\r\n")
                            self.insertARow([self.knew_id[i],self.knew_name[i],nowdt,"否"],2)
                        else:
                            self.infoText.AppendText(self.knew_name[i] + " 成功签到,但迟到了\r\n")
    ...
    def initGallery(self):
        self.pic_index = wx.Image("resources/index.jpg", wx.BITMAP_TYPE_ANY).Scale(600, 500)
    ...
    def initDatabase(self):
        conn = sqlite3.connect("facedetect_signin_demo.db")  #建立数据库连接
    ...
    def insertARow(self,Row,type):
        conn = sqlite3.connect("facedetect_signin_demo.db") 
        cur = conn.cursor() 
        if type == 1:
